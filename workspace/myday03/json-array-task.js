...
file.readFile('shop.json', 'utf-8', (error, content) => {
    if(error){
        ...
        function Total(totalPrice, totalStock){
        ...
        total.totalPrice = products.map(product => ps = product.price * product.stock).reduce((total, ps) => total + ps);
        total.totalStock = products.map(product => stock = product.stock).reduce((total, stock) => total + stock);


        // totalJSON = JSON.stringify(new Total(totalPrice, totalStock));
